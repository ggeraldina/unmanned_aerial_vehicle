...
class Video:
    ...
    def run(self):
        ...
        while(True):
            ...
            self._drow_contours(frame, foreground_mask)
    ...
    def _drow_contours(self, frame, foreground_mask):
        contours, hierarchy = cv2.findContours(
            foreground_mask.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        # Drow contours
        self._drow_framing_contours(frame, contours, hierarchy)
        # OR drow framing rectangles
        self._drow_framing_rectangles(frame, contours)


    def _drow_framing_rectangles(self, frame, contours):
        ...
        # Text on the frame
        self._show_count(frame, amount_drawn_contours)


    def _show_count(self, frame, amount_drawn_contours):
    ...
    def _drow_framing_contours(self, frame, contours, hierarchy):
        contour_index = -1
